...
class PurchaseOrder(models.Model):
    ...
    def _onchange_edocument_id(self):
        ...
        order_lines = []
        lines_to_process = edocument.document_line
        lines_to_unify = edocument.document_line.filtered(lambda line: line.unificar)
        # lines_to_distribute = edocument.document_line.filtered(lambda line: line.distribuir_code)
        lines_to_distribute = edocument.document_line.filtered(lambda line: line.distribuir_code and line.distribuir_code.strip())
        print("-"*50)
        ...
        for line in lines_to_process - lines_to_unify - lines_to_distribute:
            ...
            # # Create PO line
            order_line_values=line._prepare_purchase_order_line( line.product_id, product_qty, line.product_uom,line.discount, line.company_id, partner_id, self)
...
class PurchaseOrderLine(models.Model):
    ...
    def _compute_price_unit_and_date_planned_and_name(self):
        ...
        for pol in self:
            if pol.product_id.id not in pol.order_id.edocument_id.document_line.product_id.ids:
                po_lines_without_requisition |= pol
                continue
            for line in pol.order_id.edocument_id.document_line:
